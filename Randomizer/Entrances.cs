	...
	partial class Randomizer
    {
		...
		public static Dictionary<string, string> ProcessEntrances(Dictionary<string, string> newTeleporters)
		{
			...
			foreach (var teleporter in newTeleporters)
			{
				...
				{
					InternalLogger.LogWarning($"Entrances: {teleporter.Key} not found.");
				}

				if (!Randomizer.Teleporters.TryFind(t => t.Name == Randomizer.TeleporterPairs[teleporter.Value], out tele))
				{
					InternalLogger.LogWarning($"Entrances: {Randomizer.TeleporterPairs[teleporter.Value]} not found.");
				}

				if (!Randomizer.Teleporters.TryFind(t => t.Name == Randomizer.TeleporterPairs[teleporter.Key], out tele))
				{
					InternalLogger.LogWarning($"Entrances: {Randomizer.TeleporterPairs[teleporter.Key]} not found.");
				}

				if (!Randomizer.Teleporters.TryFind(t => t.Name == teleporter.Value, out tele))
				{
					InternalLogger.LogWarning($"Entrances: {teleporter.Value} not found.");
				}
