	...
	***/
	...
	public static class Options
	{
		...
		private static List<Option> optionlist = new()
		{
			...
			new("shuffle_overworld", "Shuffle Overworld", new() { { 1, "Enable" }, { 0, "Disable" } }, 0, "Shuffle all Overworld entrances, except the Towns."),
			new("shuffle_entrances", "Shuffle Entrances", new() { { 0, "No Shuffle" }, { 1, "Dungeon Internal" }, { 2, "Dungeon Mixed" }, { 3, "All" } }, 0, "Shuffle all non-Overworld entrances.\n\nNo Shuffle: Entrances stay the same.\n\nDungeon Internal: Only shuffle the Dungeons' entrances, and keep them inside their respective dungeon.\n\nDungeon Mixed: Only shuffle the Dungeons' entrances amongst all dungeons.\n\nAll: Shuffle all locations' entrances."),
			new("shuffle_towns", "Shuffle Towns", new() { { 0, "No Shuffle" }, { 1, "Between Towns" }, { 2, "Shallow Shuffle" }, { 3, "Deep Shuffle" } }, 0, "Shuffle all Town entrances, except Cornelia; if Early Progression is set to Bikke's Ship, Pravoka is also excluded.\n\nNo Shuffle: Entrances stay the same.\n\nBetween Towns: Towns are shuffled only amongst themselves.\n\nShallow Shuffle: If Overworld or Entrances are shuffled, Towns will be included with them, but will always be on the first floor of any location.\n\nDeep Shuffle: If Overworld or Entrances are shuffled, Towns will be included with them and can end up at any floor inside a location."),
